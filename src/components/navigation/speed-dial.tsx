...
function toButtonClickEvent(
  event: React.MouseEvent<HTMLButtonElement>,
): Parameters<ButtonClickHandler>[0] {
  return event as Parameters<ButtonClickHandler>[0]
}
...
function SpeedDialTrigger(props: React.ComponentProps<typeof Button>) {
  ...
  const onClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      onClickProp?.(toButtonClickEvent(event))
      if (event.defaultPrevented) return
  ...
  const onMouseEnter = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      onMouseEnterProp?.(
        event as Parameters<
          NonNullable<React.ComponentProps<typeof Button>["onMouseEnter"]>
        >[0],
      )
  ...
  const onMouseLeave = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      onMouseLeaveProp?.(
        event as Parameters<
          NonNullable<React.ComponentProps<typeof Button>["onMouseLeave"]>
        >[0],
      )
      if (event.defaultPrevented || activationMode !== "hover" || isDisabled)
        return
...
function SpeedDialAction(props: SpeedDialActionProps) {
  ...
  const onClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      propsRef.current?.onClick?.(toButtonClickEvent(event))
      if (event.defaultPrevented) return
...
function SpeedDialLabel({ asChild, className, ...props }: DivProps) {
  ...
  const LabelPrimitive = asChild ? SlotPrimitive : "div"
