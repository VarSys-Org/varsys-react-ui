...
export const DottedGlowBackground = ({
  className, gap = 12, radius = 2,
  color = "rgba(0,0,0,0.7)", darkColor,
  glowColor = "rgba(0, 170, 255, 0.85)", darkGlowColor,
  opacity = 0.6, backgroundOpacity = 0,
  speedMin = 0.4, speedMax = 1.3, speedScale = 1,
  ...
  useEffect(() => {
    ...
    const regenDots = () => {
      ...
      for (let i = -1; i < cols; i++) {
        for (let j = -1; j < rows; j++) {
          ...
          dots.push({ x, y, phase, speed: min + Math.random() * span })
        }
      }
    }
    ...
    window.addEventListener("resize", () => { resize(); regenDots() })
    raf = requestAnimationFrame(draw)
...
DottedGlowBackground.displayName = "DottedGlowBackground"
