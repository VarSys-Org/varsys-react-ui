...
import { VisuallyHiddenInput } from "@/components/forms/visually-hidden-input"
...
function KeyValueRemove(props: KeyValueRemoveProps) {
  ...
  const propsRef = useAsRef({
    onClick: onClickProp as unknown as React.MouseEventHandler<RemoveElement>,
  })
  const value = useStore((state) => state.value)
...
function KeyValueAdd(props: React.ComponentProps<typeof Button>) {
  ...
  const propsRef = useAsRef({
    onClick: onClickProp as unknown as React.MouseEventHandler<AddElement>,
  })
  const value = useStore((state) => state.value)
  const isDisabled =
    context.disabled ||
