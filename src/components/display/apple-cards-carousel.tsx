...
function Carousel({ items, initialScroll = 0 }: CarouselProps) {
  ...
  return (
    <CarouselContext.Provider value={{ onCardClose: handleCardClose, currentIndex }}>
      <div data-slot="apple-cards-carousel" className="relative w-full">
...
function AppleCard({ card, index, layout = false }: { card: Card; index: number; layout?: boolean }) {
  const [open, setOpen] = useState(false)
  const containerRef = useRef<HTMLDivElement>(null)
  const { onCardClose } = useContext(CarouselContext)

  useClickOutside(containerRef as React.RefObject<HTMLDivElement | null>, () => handleClose())
...
export { Carousel, AppleCard }
export type { Card as AppleCardType }
