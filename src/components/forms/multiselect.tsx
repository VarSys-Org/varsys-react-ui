      ...
      const Item = (
        <CommandItem
          ...
          onSelect={() => {
            ...
            setInputValue("")
            const newOptions = [...selected, { label: inputValue, value: inputValue }]
            setSelected(newOptions)
            onChange?.(newOptions)
          }}
          value={inputValue}
          ...
          <div className="flex flex-wrap gap-1">
            {selected.map((option) => {
              return (
                <div
                  className={cn(
                    "relative inline-flex h-7 cursor-default items-center rounded-md border bg-background ps-2 pe-7 pl-2 font-medium text-secondary-foreground text-xs transition-all hover:bg-background disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pe-2",
                    badgeClassName
                  )}
