    ...
    <div className="flex aspect-3/6 flex-col overflow-hidden rounded-[2px] border border-neutral-300 md:rounded-[3px] md:border-2 dark:border-black">
      <div className="relative flex-1 perspective-dramatic transform-3d">
        ...
        {flipId > 0 && (
          <motion.div
            key={flipId}
            className={cn("absolute inset-x-0 top-0 z-10 h-[calc(50%-0.5px)] origin-bottom overflow-hidden rounded-t-[3px] backface-hidden transform-3d", flapTopBg)}
            initial={{ rotateX: 0 }}
            animate={{ rotateX: -100 }}
        ...
        {flipId > 0 && (
          <motion.div
            key={`b${flipId}`}
            className={cn("absolute inset-x-0 bottom-0 z-10 h-[calc(50%-0.5px)] origin-top overflow-hidden rounded-b-[3px] backface-hidden transform-3d", bottomBg)}
            initial={{ rotateX: 90 }}
            animate={{ rotateX: 0 }}
...
(prevProps, nextProps) =>
  prevProps.target === nextProps.target && prevProps.delay === nextProps.delay && prevProps.stepMs === nextProps.stepMs && prevProps.flipDuration === nextProps.flipDuration)
...
const ColorCell = React.memo(function ColorCell({ color }: { color: string }) {
  return <div className="aspect-3/5 rounded-[3px] border-2 border-neutral-300 dark:border-black" style={{ backgroundColor: color }} />
})
...
function parseRow(row: string): ParsedCell[] {
  ...
  while (i < row.length) {
    if (row[i] === "{" && i + 2 < row.length && row[i + 2] === "}") {
      ...
      if (COLOR_MAP[code]) { cells.push({ type: "color", hex: COLOR_MAP[code] }); i += 3; continue }
    }
...
function wrapParagraph(paragraph: string, maxCols: number): string[] {
  ...
  for (const word of words) {
    if (word.length > maxCols) {
      ...
      lines.push(word.slice(0, maxCols)); continue
    }
...
function TextFlippingBoard({ rows, text, className, duration = BASE_TOTAL_S }: TextFlippingBoardProps) {
  ...
  return (
    <div data-slot="text-flipping-board" className={cn("relative mx-auto w-full max-w-3xl rounded-xl bg-neutral-100 p-2 shadow-xl md:rounded-2xl md:p-4 dark:bg-neutral-900 dark:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]", className)}>
