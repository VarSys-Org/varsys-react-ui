...
export function MaskedHeading({
  ...
  return (
    <TagAny
      ref={rootRef}
      className={cn("relative m-0 w-full p-0 antialiased [text-wrap:balance]", className)}
      style={{
        textAlign: align,
        fontWeight: weight,
        ...
        <span className="absolute inset-0 block" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="absolute inset-0 block [will-change:transform,filter]">
            {mediaType === "video" ? (
              ...
              <img className="block h-full w-full select-none object-cover" src={src} alt="" draggable={false} />
            )}
          </span>
        </span>
      </span>
